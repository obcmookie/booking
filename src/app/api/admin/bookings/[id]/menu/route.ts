...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data: b, error: bErr } = await supa
    .from("bookings")
    .select(
      "id,event_type,status,requested_start_date,requested_end_date,event_date,menu_template_id,menu_customer_token",
    )
    .eq("id", id)
    .single();
  ...
  type MenuItemLite = { id: string; name: string; category_id: string };
  let items: MenuItemLite[] = [];
  if (itemIds.length > 0) {
    ...
    items = (itemsData ?? []) as MenuItemLite[];
  }

  const catIds = Array.from(new Set(items.map((i) => i.category_id)));

  type MenuCatLite = { id: string; name: string };
  let cats: MenuCatLite[] = [];
  if (catIds.length > 0) {
    const { data: catsData, error: catsErr } = await supa
      .from("menu_categories")
      .select("id,name")
      .in("id", catIds);
    if (catsErr) return NextResponse.json({ error: catsErr.message }, { status: 400 });
    cats = (catsData ?? []) as MenuCatLite[];
  }

  const itemName = new Map<string, string>(items.map((i) => [i.id, i.name]));
  const catById = new Map<string, string>(cats.map((c) => [c.id, c.name]));
  ...
  const info: BookingMenuInfo = {
    ...
    selections: (selections ?? []).map((s) => {
      const catId = items.find((i) => i.id === s.menu_item_id)?.category_id ?? "";
      return {
        item_id: s.menu_item_id,
        item_name: itemName.get(s.menu_item_id) ?? "(item)",
        category_name: catById.get(catId) ?? "",
        qty: s.qty,
...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // No generics — cast the data after the call to satisfy typing
  const { data, error } = await admin().rpc("rpc_open_menu", { p_booking_id: id });
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });

  const token = (data as { menu_token: string | null } | null)?.menu_token ?? null;
