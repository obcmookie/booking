...
import { BookingMenuInfo } from "@/components/types";

export default function BookingMenuPage({ params }: { params: { id: string } }) {
  ...
  const menuLink = useMemo(() => {
    ...
    return `${baseUrl}/menu/${info.menu_token}`;
  }, [baseUrl, info?.menu_token]);
      ...
      <div className="rounded border p-3">
        ...
        {menuLink ? (
          <div className="flex items-center gap-3 text-sm">
            ...
            <Link target="_blank" href={menuLink} className="underline">
              Open
            </Link>
          </div>
        ) : (
